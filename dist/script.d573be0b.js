...
(function () {
  var BLOCK_CLASS = 'calculator';
  var SELECTOR_CLASS = {
    DISPLAY: ".".concat(BLOCK_CLASS, "__display")
  };
  var displayElement = document.querySelector(SELECTOR_CLASS.DISPLAY);

  var calculatorButtons = _toConsumableArray(document.querySelectorAll(".".concat(BLOCK_CLASS, " > button")));
  ...
  var setButtonActions = function setButtonActions() {
    calculatorButtons.forEach(function (button) {
      button.addEventListener('click', function (event) {
        var typeAction = event.target.dataset.type || '';
  ...
  function handleDisplayValue(event) {
    ...
    CALCULATOR.currentValue = CALCULATOR.currentValue === '0' ? event.target.value : CALCULATOR.currentValue + event.target.value;
    displayElement.value = CALCULATOR.currentValue;
  }
  ...
  function calculate(event) {
    if (CALCULATOR.currentValue === '0' || CALCULATOR.isEqualPressed && CALCULATOR.operator.length === 0) {
      return;
    }

    var accumulator = CALCULATOR.accumulator,
    ...
    CALCULATOR.accumulator = accumulator === '0' && operator.length === 0 ? currentValue : operations[operator].toString();
    displayElement.value = CALCULATOR.accumulator;
